...
public class DragImageView extends ImageView {
	...
	public void setScreen_W(int screen_W) {
		Log.d("tigertiger","setScreen_W ="+screen_W);
		this.screen_W = screen_W;
	}

	/** 可见屏幕高度 **/
	public void setScreen_H(int screen_H) {
		Log.d("tigertiger","screen_H ="+screen_H);
	...
	public void setImageBitmap(Bitmap bm,int width,int heigth) {
		...
		this.bm = bm;
		Rect drawFrameRect = ScaleTool.getScaledPosition(bm.getWidth(), bm.getHeight(), 
				width, heigth);
...
//			}
	...
	void onTouchMove(MotionEvent event) {
		...
		if (mode == MODE.DRAG) {
			...
			/** 垂直判断 **/
			if (isControl_V) {
				Log.d("tigertiger","top =="+top);
				Log.d("tigertiger","this.getHeight() =="+this.getHeight());
				if (top >= 0) {
					top = 0;
					bottom = this.getHeight();
				}
				Log.d("tigertiger","bottom =="+bottom);
				Log.d("tigertiger","this.getHeight() =="+this.getHeight());
				Log.d("tigertiger","screen_H =="+screen_H);
				if (bottom <= screen_H) {
	...
	void setScale(float scale) {
		...
		if (scale > 1 && this.getWidth() <= MAX_W) {
			...
			Log.d("tigertiger","****************isControl_V ="+isControl_V);
		}
		// 缩小
		else if (scale < 1 && this.getWidth() >= MIN_W) {
			Log.d("tigertiger","222222****************MIN_W ="+MIN_W);
			current_Left = this.getLeft() + disX;
			current_Top = this.getTop() + disY;
			current_Right = this.getRight() - disX;
			current_Bottom = this.getBottom() - disY;
